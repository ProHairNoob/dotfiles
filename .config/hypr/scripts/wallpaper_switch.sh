...
SELECTED_NAME=$(printf "%s\n" "${FILENAMES[@]}" | rofi -dmenu -p "Select a wallpaper")
...
# If a wallpaper was selected, set it using awww, apply the colors with pywal, and update spicetify
if [ -n "$SELECTED" ]; then
  awww img "$SELECTED" >>/tmp/wallpaper_switcher.log 2>&1
else
